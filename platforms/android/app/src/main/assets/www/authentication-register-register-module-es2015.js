...
let RegisterPage = class RegisterPage {
    constructor(utils, router, http, service, formBuilder) {
        this.utils = utils;
        this.router = router;
        this.http = http;
    ...
    mismatchedPasswords(otherControlName) {
        return (control) => {
    ...
    checkUsername() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            if (item) {
                yield this.utils.presentLoading("Please wait...");
                this.http.get(`${this.service.homeUrl}/users/username/${item}`, { observe: 'response' }).subscribe(response => {
                    this.utils.stopLoading();
                    if (response.status === 200 || response.status === 201) {
                        const tester = response.body;
                        if (tester.toString() === 'false') {
...
//# sourceMappingURL=authentication-register-register-module-es2015.js.map